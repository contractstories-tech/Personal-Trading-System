...
**Resolution:** every finding is resolved in `equity-spec-kit` release r5.5 and carried forward to the current release, r5.10 (in this repository, and as `equity-spec-kit-r5.10.zip`). Each finding's disposition is in `equity-spec-kit/docs/Issue-Log-and-Traceability-r5-10.md` §12, and later reviews are in §13–§17. The scripts in `repro/` target the r5.4 package; each release guards the fixes with its own suites. For example, the A1 card edits are planted edits in `test_card_golden.py`, and the B2 bypasses are regression cases in `test_speclint.py`.

...
from speclint import load_yaml, sha256_file, load_transitions, lifecycle_status  # noqa: E402
...
def main():
    ...
    ap.add_argument("--at", default=None, help="RFC 3339 timestamp; default now")
    a = ap.parse_args()
    ...
    out = subprocess.run([sys.executable, os.path.join(HERE, "speclint.py"), a.card], capture_output=True, text=True)
    if out.returncode != 0:
        sys.exit("the card does not pass the linter:\n" + out.stdout)
    report = os.path.join(HERE, "lifecycle", "reports", f"{code}-{card['version']}.lint.txt")
    open(report, "w").write(out.stdout)
    n = len([f for f in os.listdir(os.path.join(HERE, "lifecycle", "transitions")) if f.endswith(".json")]) + 1
    at = a.at or dt.datetime.now(dt.timezone(dt.timedelta(hours=5, minutes=30))).isoformat(timespec="seconds")

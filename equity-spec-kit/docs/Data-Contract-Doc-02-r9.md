# Data Contract & Canonical Schema — Document 02 r9

*Release r5.10 · 26 September 2026 · current state only; revision history is in the Issue Log*

Governs every table, field, source, timing rule and feature definition referenced by Document 01 r12 and the strategy cards. r5.8 incorporated the first real NSE UDiFF/MII findings; r5.9 extends that contract to delivery-source validation, source acquisition/coverage evidence and non-semantic price-band capture. A card owns its thresholds; how the compared value is computed is defined here and in `registry.yaml`. An identifier a card uses that is not in the registry fails the build.
...
**r5.9 acquisition and coverage.** `eos.m2.catalog` is the explicit allowlist of NSE sources and archive URL/filename conventions; `eos.m2.acquire` downloads bytes only and never mutates the warehouse. It refuses:
- any non-https URL, or any host (including every redirect target and the final URL) outside NSE;
- a response that is not the source's real file: an HTML page (anti-bot answers can be HTTP 200), a zip or gzip source without its magic bytes, or binary where text is expected;
- a payload above the size cap;
- a live-URL source (the price-band list) for any date other than the retrieval date in IST, since that URL always serves the current list.

Requests are paced by a minimum interval. An existing download is never overwritten: identical bytes are reused, and a reissue is saved as `<stem>.reissue-<sha12><ext>`. The bhavcopy URL follows the layout in use on the date (legacy `cmDDMONYYYYbhav.csv.zip` before `catalog.UDIFF_ONLY_FROM`, UDiFF after; the cut-over date is unverified and only chooses the URL). `--variant` overrides it. Month names come from a fixed table, never the process locale (r5.10). The bytes still enter research state only through the normal landing/receipt/parser path. `coverage_report.py` opens an existing warehouse read-only (a wrong path is an error, never a new empty warehouse) and covers weekdays unless asked for weekends; until the trading calendar exists, a weekday holiday still reads as missing. It reports, for each requested date/source, `parsed_complete`, `parsed_incomplete`, `captured_unparsed`, `parse_rejected`, `parsed_without_coverage`, `receipt_only` or `missing`. A capture-only source can never become parsed coverage by receipt alone.

**Delivery evidence.** `nse_cm_delivery` (MTO) remains the primary delivery source. Its type-10 trade date and declared detail count are structural checks; type-20 reported deliverable percentage is retained and arithmetically compared with quantity/traded quantity. The full bhavcopy deliverable file is stored separately as `delivery_crosscheck_observation`: it cannot overwrite or populate the primary MTO table. When both exist for a session, whichever arrives second compares them per (ISIN, series). It logs `delivery_source_mismatch` (deliverable quantity), `delivery_traded_qty_mismatch`, or `delivery_availability_mismatch` (MTO has a value where full bhav shows `-`). The MTO value stays primary and neither is corrected (r5.10; r5.9 described the comparison but did not perform it). A `-` delivery quantity/percentage is `delivery_available=false`, never zero.
...
**Source observations.** `price_observation` keeps the exchange observation, not a prematurely collapsed security/day value: `source_instrument_id` (nullable for legacy files) · `isin` · `symbol` · `series` · `trade_date` · `version_no` · OHLC · `prev_close` · volume · traded value · trades · source/provenance/availability fields · `supersedes_version`. **Observation identity is `(isin, series, trade_date)` in every file format** (r5.10, `eos/m2/identity.py`). `source_instrument_id` (UDiFF `FinInstrmId`) is stored on the observation as the join key to the MII master; it does not decide whether two rows are the same observation. r5.8–r5.9 keyed UDiFF rows by `FinInstrmId` and legacy rows by ISIN+series, which gave one instrument two identities on the same date. A correction delivered in the other format then counted as a *first* version and took the inferred trade-date availability, so a price received days later became usable on the trade date, and the canonical read failed with two candidates. Now the same content in either format is one observation (no new version), and a changed value is a correction, available only when received. Within one file, the same `FinInstrmId` twice, or the same (ISIN, series) under two IDs, is a duplicate and is quarantined. **The same ISIN in different series on one date (EQ and BL) is legitimate** and retained.
...
**Corrections and withdrawals.** A changed observation is a new immutable version. A row absent from a later file for the same date is **withdrawn** only when both hold:
- the source policy declares that source's reissues complete snapshots (`reissue_semantics: complete_snapshot`);
- the absent row came from a file of the **same format**.

Only then does the system write `observation_tombstone` with the identity, withdrawal version, hash and `usable_from`. Otherwise the prior version is kept and `absent_in_reissue` is logged. Every shipped source is `unverified` until real reissues prove the semantics. r5.8–r5.9 tombstoned on any absence, including absence from a file of the other format, whose row set differs. History is never deleted. Reads before the correction see the old row; reads after it do not. A quarantined row is not treated as an omission and therefore cannot accidentally tombstone a prior valid observation.

**Delivery is independent.** `delivery_observation` is versioned separately and may carry the mapped `source_instrument_id`; delivery arrival/corrections never rewrite the price row. Its rows map through the date's **active** price observations; a withdrawn price row is never a mapping target (r5.10).
...
2. **Parse/observation transaction.** Parsing then records a `raw_parse_event` (`parsed` or `rejected`, format/date when known, and parser error when rejected) and, on success, observations, conflicts, quarantine, coverage and ingestion log atomically. A structural/parser rejection therefore leaves bytes + receipt + rejected parse evidence, but no observations. Every landed file ends with a recorded outcome: a file that parses but then fails a quality check (backfill guard, quarantine limit, kill switch, expected date, no bhavcopy for the day) also records `rejected`, with its format and trade date (r5.10; r5.7–r5.9 left such files as a receipt with no outcome, indistinguishable from a crash between the two transactions).
...
**Every ingestion:** OHLC ordering and identifier validity per row (a failing row is quarantined, §6; the file is rejected above the quarantine limit); uniqueness of the observation identity `ISIN + series + trade_date` in every format, and of `FinInstrmId` within a UDiFF file; legitimate same-ISIN multi-series rows preserved; MII source-instrument IDs unique within a snapshot; exact source schema fingerprints; row count within ±5% of the prior day (kill switch below 90%); no unresolved master state used to manufacture eligibility/no-trade; tombstones only from same-format reissues of a source whose reissues are proven complete snapshots; close-to-close moves beyond 3× the band with no corporate action flagged for review; adjusted-series continuity at ex-dates; the financial/share-count reconciliations; `usable_from ≥ filed_at` everywhere.
